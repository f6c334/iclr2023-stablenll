...
MODEL_CONFIG = {
  'AdamModel':
    lambda hparams, **kwargs: m.AdamModel(**SHARED_MODEL_PARAMETERS,
                                          ...
                                          covariance_head_type='softplusstd',
                                          **kwargs),
  'Pitfalls05Model':
...
if __name__ == '__main__':
  ...
  logdir = os.path.join(ROOT_DIR, 'uci_univariate', function_id, model_id)
  
  # prepare data
  X, Y, (num_samples, num_features, num_outputs) = uci.get(set_id=function_id)
  Y = np.expand_dims(Y, axis=-1)

  build_model = lambda hparams, **kwargs: MODEL_CONFIG[model_id](hparams, input_shape=[num_features], n_dims=num_outputs, **kwargs)
