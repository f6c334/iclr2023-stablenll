...
def traptable_gaussian_optimization(initial_mean, initial_covariance, 
                              true_mean, true_covariance, 
                              phi, alpha, proj_layer,
                              iterations, batch_size, mini_batches):
  
  def _optimize(train_dataset, p_mean, p_covariance, delta, M, phi, alpha, mean_old, covariance_old, proj_layer):
    for _, minibatch in enumerate(train_dataset):
      ...
      proj_mean, proj_covariance = proj_layer(new_mean, mean_old, new_covariance, covariance_old)
  ...
  for i in range(iterations):
    ...
    _optimize(train_dataset, p_mean, p_covariance, delta, M, phi, alpha, mean_old, covariance_old, proj_layer)
...
if __name__ == '__main__':
  ...
  normal_config = case_configs[1][6]
  initial_mean, initial_covariance, true_mean, true_covariance = normal_config.values()

  phi = parametrizations.ApproximateTractableCholeskyParametrization()
  proj_layer = trust_region_layers.W2ProjectionLayer(mean_bound=1.0, covariance_bound=1.0)
  ...
  ################### PLOTTING TEST
  seeding(SEED, tf_deterministic=False)
  log = traptable_gaussian_optimization(initial_mean, initial_covariance,
                                  true_mean, true_covariance,
                                  phi, alpha, proj_layer,
                                  iterations, batch_size, mini_batches)
